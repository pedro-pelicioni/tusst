...
// skirmish has a node, every node points at real content, the roads reach
// every node, and no two nodes sit on top of each other. Run:
//
...
console.log(
  `overworld OK: ${JOURNEY_WORLD.nodes.length} journey nodes, ${CAMPAIGN_WORLD.nodes.length} campaign nodes, ${WORLD_MAP.nodes.length} world-map doors`,
);
