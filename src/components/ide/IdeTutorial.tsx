...
// step's `data-tutorial-id` target in IdeShell. The step list comes from the
// caller: the desktop and compact (mobile) layouts walk different targets.
// Persistence lives in forge-store.

export interface TutorialStep {
  /** Matches a `data-tutorial-id` attribute in the active layout. */
  key: string;
  title: string;
  body: string;
}
...
function measureTarget(stepKey: string): DOMRect | null {
  ...
  // A hidden target (display:none) measures 0×0 — treat it as absent so the
  // popover centers instead of pointing at the viewport corner.
  return rect.width > 0 ? rect : null;
}

export function IdeTutorial({
  steps,
  onDone,
}: {
  ...
  const rect = useMemo(() => measureTarget(step.key), [step.key, resizeTick]);

  const isLast = index === steps.length - 1;

  // Phone viewports are narrower than the design width — shrink to fit.
    ...
    <>
      ...
      >
        <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-accent/70">
          {fmt(m.ide.tutorial.stepCounter, { current: index + 1, total: steps.length })}
        </p>
