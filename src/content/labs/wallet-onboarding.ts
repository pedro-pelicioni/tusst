...
No form. No email. No permission. You *forge* an identity out of pure math — and in the next few minutes it will hold funds, trust an asset, and pay another account. All of it real, on the **testnet**: Stellar's practice network, where the coins are play coins but the machinery is the same.`,
    },
    {
      kind: "action",
      id: "forge-keys",
...
That address is public — share it freely. The secret key beneath it signs on your behalf; anyone holding it *is* you. On testnet that's fine. On mainnet, never share the secret key.`,
    },
    {
      kind: "action",
      id: "friendbot",
...
On testnet, a faucet called **Friendbot** funds anyone who asks.`,
...
Notice what it cost: a tiny fee (~0.00001 XLM) and one more base reserve locked, because a trustline is a new ledger entry. Your sequence number just ticked up, too.`,
    ...
    },
    {
      kind: "action",
      id: "shrine",
      title: "Create a second address",
      body: `You can't send a payment into the void — you need a **destination**. Let's create a second address to receive your first payment.

We'll generate it and *throw the secret key away*. The account will exist and hold what you send it, but nobody can ever sign for it.`,
      cta: "Create the address",
      action: { type: "generate-keypair", target: "state", stateKey: "companion" },
      successBody: `The second address:
...
It doesn't exist on the ledger yet — same as yours before Friendbot. But this time **you** will be the one bringing it to life.`,
    },
    {
      ...
      body: `A \`create_account\` operation funds a new address past the base reserve — precisely what Friendbot did for you. Now you do it for the second address, from **your** balance: 100 testnet XLM.`,
      cta: "Create it (send 100 XLM)",
      action: {
        type: "classic-op",
        ops: (ctx) => [
          {
      ...
      successBody: `Account created. You just did what Friendbot did for you — **accounts create accounts**. That's the whole hierarchy; there is no registrar.`,
      explorer: "tx",
    },
    {
      kind: "action",
      ...
      successBody: `Payment sent — 25 XLM, final, irreversible, on public record:
...
Fee, sequence bump, two balances updated, one ledger close. Five seconds. That's a Stellar payment.`,
    ...
    },
    {
      ...
      answer: 0,
      explain: `Right. Trustlines are per-account, per-asset. Your main account trusts USDC; the second account only holds native XLM. And since its secret was thrown away, no one can ever open one for it.`,
    },
    {
      kind: "checkpoint",
      id: "claim",
