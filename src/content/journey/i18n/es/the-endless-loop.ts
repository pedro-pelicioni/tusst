...
El modelo escribe código, lo *ejecuta*, lee la queja del compilador, corrige, ejecuta otra vez — como trabajas tú, a ritmo de máquina. La calidad de un solo tiro dejó de ser el número interesante en cuanto el modelo pudo ver sus propios resultados.
...
Señales, no sensaciones. "La salida parece razonable" no corrige nada, porque nunca puede ser falso. Cada verificador que metiste en el arnés ahora da intereses: conectado al bucle, se convierte en los ojos con los que se guía el modelo — **en cada iteración**.` },
    { kind: "quiz", question: `¿Qué observación puede dirigir de verdad un bucle?`,
      options: [
        ...
        "El resumen final del propio modelo: ahora todo parece correcto",
        "Que el código compilara a la primera — fuerte evidencia de que la lógica está bien",
      ], answer: 0,
      explain: `Compilar significa que los tipos encajan, no que el comportamiento sea el deseado — y un autorresumen es la mente corrigiendo sus propios deberes. Una señal de dirección debe ser externa, específica y capaz de ser mala noticia. "1 falló, línea 41" te dice hacia dónde ir; "parece correcto" no te dice nada.` },
...
**Actuar.** El modelo edita \`refunds.rs\` — cambia la comparación de plazo de \`>\` a \`>=\`. Un solo cambio, porque un turno que cambia seis cosas no puede decirte cuál funcionó.
...
Fíjate en qué hizo que ese turno valiera algo. No fue el modelo quien decidió que había mejorado. **Fue el recuento.**` },
...
Con una brújula, el bucle sabe *de hecho* si se movió: 4 verdes de 7 pasaron a 6 de 7. Sin ella, solo sabe que se movió. El progreso se **mide, no se siente**.` },
    { kind: "fill", prompt: `Completa la propiedad que hace de una brújula una brújula:`,
      ...
      explain: `Una vara que se mueve no mide nada. Por eso también "deja que el modelo escriba sus propias pruebas sobre la marcha" destruye la señal en silencio: el examen y el alumno dejan de ser cosas distintas.` },
...
3. Dice qué cuenta como TERMINADO en términos de esas señales, no de la opinión del modelo.
...
**A continuación:** los frenos, y la única ejecución en la que descubres para qué estaban.` },
  ...
  testOut: [
    { question: `¿Qué sustituye un bucle agéntico, comparado con el prompting de un solo tiro?`,
      options: ["La esperanza — el modelo ve ahora el resultado de su propio trabajo y corrige contra él","La necesidad de una especificación, ya que el bucle descubre los requisitos sobre la marcha","El compilador, ya que el bucle revisa el código por sí mismo"], answer: 0 },
