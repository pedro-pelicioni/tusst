...
export const conceptText: JourneyConceptText = {
  title: "Mínimo privilegio y rutas de fallo",
...
Un modelo con \`rm -rf\` disponible es un modelo que *eventualmente* lo ejecutará — no por malicia, sino por un plan erróneo y confiado a las 2 a.m. La solución es antigua y probada: **menor privilegio**.
...
Conceder poder "por si acaso" es cómo empiezan los incidentes. Cada herramienta tiene un radio de explosión; concédele según corresponda.`,
    ...
    { kind: "widget", component: "blast-radius",
      body: `Dos medidores, y no se mueven juntos. **Concédele al modelo lo que necesita una tarea de arreglar-y-probar**, y luego sigue añadiendo — y mira qué barra responde.` },
    {
      kind: "fill",
      prompt: `Delimita el poder del modelo antes de que empiece a trabajar:`,
      file: "harness.toml",
      before: `signing_keys = "`,
      after: `"`,
      choices: ["testnet", "mainnet", "all-networks", "treasury"],
      answer: 0,
      explain: `Regla práctica: un modelo solo posee claves cuya pérdida total puedas encarar con indiferencia. Los lumens de testnet son gratuitos gracias a friendbot; una clave de mainnet o del tesoro dentro de un bucle automatizado es un incidente con cuenta regresiva.`,
    },
...
La auditoría es barata y nadie la hace: **enumera lo que el modelo tiene hoy y, para cada cosa, nombra la tarea que la necesitó.** Todo lo que quede sin nombre en esa columna es una concesión que nadie recuerda haber hecho.` },
    { kind: "quiz",
      question: `Añades red abierta y escritura en cualquier sitio a un modelo que ya lee el repositorio, ejecuta pruebas, escribe en un directorio y tiene claves de testnet. ¿Qué compraron esas dos concesiones?`,
      options: [
...
Los aficionados diseñan lo que ocurre cuando el modelo está *correcto*. Los ingenieros diseñan lo que ocurre cuando está **equivocado** — porque a veces lo estará.

- Un chequeo fallido **bloquea el merge**; no registra una advertencia en el vacío.
- Los reintentos tienen un **presupuesto**, así un modelo atascado se convierte en un modelo detenido, no en una factura.
...
Para cada paso del arnés, haz una pregunta: *"cuando esto está mal, ¿qué lo captura?"* Si la respuesta es "ojalá que nada salga mal" — es un deseo, no un diseño.`,
    ...
    { kind: "diagram",
      ...
      view: { kind: "compare",
        ...
        rows: [
          ...
          { label: "cuando el modelo se equivoca", cells: [{ text: "sigue adelante, seguro de sí", tone: "bad" }, { text: "se detiene en el cable trampa", tone: "good" }] },
        ...
        ] } },
    {
      ...
      options: [
        ...
        "El prompt instruye firmemente al modelo a ser extremadamente cuidadoso y a doble‑chequear todo",
...
Esa es la disciplina en una imagen: construye el banco de modo que equivocarse sea *detectable* y acertar sea *demostrable* — para modelos y para humanos.

**A continuación:** las palabras mismas — lo que el modelo ve de verdad en el banco.` },
  ],
  testOut: [
    { question: `¿Por qué dar claves de testnet y no de mainnet a un bucle automatizado?`,
      options: ["Un modelo solo debe tener claves cuya pérdida total puedas encogerte de hombros — los lumens del friendbot son gratis, una clave de tesorería es un incidente con cuenta atrás","Los SDK rechazan las claves de mainnet en contextos automatizados","Las transacciones de testnet son más rápidas, así que el bucle itera antes"], answer: 0 },
    { question: `¿Cuál de estos es un camino de fallo diseñado?`,
      options: ["Una suite en rojo bloquea el auto-merge, y un humano recibe el diff más la salida del fallo","El prompt instruye firmemente al modelo a tener cuidado y comprobarlo todo dos veces","El bucle repite la misma tarea sin límite hasta que algo pase"], answer: 0 },
    ...
    { question: `Le das al modelo red abierta y permiso para escribir en cualquier sitio. ¿Qué compró eso realmente?`,
