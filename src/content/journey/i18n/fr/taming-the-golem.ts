...
Retire-lui tout et un LLM ne fait plus qu’une chose : **du texte entre, du texte sort**. Il ne peut ni exécuter du code, ni lire ton dépôt, ni vérifier la chaîne.

Tout ce qui le transforme en *travailleur* constitue son **harnais** : les outils qu’il peut appeler, les fichiers qu’il peut modifier, le bac à sable qui le contient et les vérificateurs qui jugent son résultat.
...
- **Étape de relecture** — où un humain (ou un autre modèle) inspecte le diff.
...
Le trait le plus dangereux du modèle n’est pas l’ignorance — c’est **la confiance quand il se trompe**. Il annonce le succès dans le même ton chaleureux que si le déploiement avait fonctionné ou n’avait jamais eu lieu. La confiance est un *style*, pas un signal.
...
- la **suite de tests** — tes tests, rouges ou verts
- le **linter** — les standards ont-ils été respectés ?
- la **chaîne elle‑même** — le registre dit-il ce que le modèle dit ?`,
    },
    {
      kind: "quiz",
      question: `Le modèle annonce : « Contrat déployé et initialisé avec succès. » Que fait un harnais bien conçu de cette affirmation ?`,
      options: [
        ...
        "Demande au modèle de vérifier soigneusement son propre travail dans la même session",
      ],
      answer: 0,
    ...
    { kind: "fill",
      ...
      file: "NOTES.md",
      before: `Le modèle annonce que le déploiement a réussi. Avant que cette phrase ne change quoi que ce soit, le harnais `,
      after: ` .`,
      choices: ["lit la chaîne et vérifie", "demande au modèle de le confirmer", "consigne l'affirmation dans le journal", "relance le déploiement par précaution"],
      answer: 0,
...
Vous savez désormais nommer les pièces d'un harnais et, plus important, refuser de croire quoi que ce soit que le modèle dit de son propre travail.
...
**Ensuite :** de combien de pouvoir le travail a réellement besoin, et l'unique question à poser à chaque étape que vous construisez.` },
  ...
  testOut: [
    ...
    { question: `Quel est le trait le plus dangereux du modèle ?`,
    ...
    { question: `« Contrat déployé et initialisé avec succès. » Que fait un bon harnais de cette phrase ?`,
      options: ["Il la traite comme une affirmation, lit la chaîne, appelle une fonction de lecture et croit le registre","Il l'accepte — le modèle a été fiable jusqu'ici","Il demande au modèle de revérifier son propre travail dans la même session"], answer: 0 },
  ],
};
