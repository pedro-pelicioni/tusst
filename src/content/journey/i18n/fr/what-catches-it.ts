...
export const conceptText: JourneyConceptText = {
  title: "Moindre privilège et chemins d'échec",
...
Un modèle avec \`rm -rf\` disponible est un modèle qui l’exécutera *à terme* — pas par malveillance, mais par un plan erroné confiant à 2 h du matin. Le remède est ancien et éprouvé : **moindre privilège**.
...
Les incidents commencent souvent par un pouvoir accordé « au cas où ». Chaque outil élargit le rayon d'impact ; accorde-le en conséquence.`,
    ...
    { kind: "widget", component: "blast-radius",
      body: `Deux jauges, et elles n'avancent pas ensemble. **Accordez au modèle ce dont une tâche corriger-et-prouver a besoin**, puis continuez d'ajouter — et regardez laquelle des deux répond.` },
    {
      kind: "fill",
      prompt: `Limite le pouvoir du modèle avant qu’il ne commence le travail :`,
      file: "harness.toml",
      before: `signing_keys = "`,
      after: `"`,
      choices: ["testnet", "mainnet", "all-networks", "treasury"],
      answer: 0,
      explain: `La règle empirique : un modèle ne possède que les clés dont la perte totale peut être ignorée. Les lumens testnet sont gratuits grâce à friendbot ; une clé mainnet ou trésorerie dans une boucle automatisée est un incident avec un compte à rebours.`,
    },
...
L'audit est bon marché et personne ne le fait : **listez ce que le modèle détient aujourd'hui et, pour chaque élément, nommez la tâche qui l'a exigé.** Tout ce qui reste sans nom dans cette colonne est un droit que personne ne se souvient d'avoir donné.` },
    { kind: "quiz",
      question: `Vous ajoutez un réseau ouvert et l'écriture n'importe où à un modèle qui lit déjà le dépôt, lance les tests, écrit dans un répertoire et détient des clés de testnet. Qu'ont acheté ces deux droits ?`,
      options: [
...
Les amateurs conçoivent ce qui se passe lorsque le modèle a raison. Les ingénieurs conçoivent ce qui se passe lorsqu’il a **tort** — parce que cela arrivera.

- Une vérification qui échoue **bloque la fusion** ; elle ne se contente pas de consigner un avertissement ignoré.
- Les tentatives ont un **budget**, donc un modèle bloqué devient un modèle arrêté, pas une facture.
...
À chaque étape du harnais, pose-toi cette question : *« si le résultat est faux, qu'est-ce qui le détecte ? »* Si la réponse est « espérons qu'aucun problème ne survienne », tu as formulé un vœu, pas conçu un système.`,
    ...
    { kind: "diagram",
      ...
      view: { kind: "compare",
        ...
        rows: [
          ...
          { label: "quand le modèle a tort", cells: [{ text: "il continue, sûr de lui", tone: "bad" }, { text: "il s'arrête au fil-piège", tone: "good" }] },
        ...
        ] } },
    {
      ...
      options: [
        ...
        "Le prompt instruit fermement le modèle à être extrêmement prudent et à tout vérifier",
...
Voilà la discipline en une image : construisez l'établi de sorte qu'avoir tort soit *détectable* et avoir raison soit *démontrable* — pour les modèles comme pour les humains.

**Ensuite :** les mots eux-mêmes — ce que le modèle voit réellement sur l'établi.` },
  ],
  testOut: [
    { question: `Pourquoi donner des clés de testnet plutôt que de mainnet à une boucle automatisée ?`,
      options: ["Un modèle ne doit détenir que des clés dont la perte totale vous laisse indifférent — les lumens du friendbot sont gratuits, une clé de trésorerie est un incident à retardement","Les SDK refusent les clés mainnet en contexte automatisé","Les transactions de testnet sont plus rapides, la boucle itère donc plus tôt"], answer: 0 },
    { question: `Lequel de ceux-ci est un chemin d'échec conçu ?`,
      options: ["Une suite au rouge bloque la fusion automatique, et un humain reçoit le diff plus la sortie de l'échec","Le prompt enjoint fermement au modèle d'être prudent et de tout revérifier","La boucle réessaie la même tâche sans limite jusqu'à ce que quelque chose passe"], answer: 0 },
    ...
    { question: `Vous accordez au modèle un réseau ouvert et le droit d'écrire n'importe où. Qu'est-ce que cela a acheté ?`,
