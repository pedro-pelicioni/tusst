...
// Taming the Model. The widget is the payload: capability and blast radius
...
export const whatCatchesIt: Concept = {
  meta: {
    ...
    title: "Least Privilege & Failure Paths",
...
Power granted "just in case" is how incidents start. Every tool is a blast radius; grant accordingly.`,
    },
    {
      ...
      body: `Two meters, and they do not move together. **Grant the model what a fix-and-prove task needs**, then keep adding — and watch which bar responds.`,
    },
    {
      kind: "fill",
      prompt: `Scope the model's power before it starts work:`,
      file: "harness.toml",
      before: `signing_keys = "`,
      after: `"`,
      choices: ["testnet", "mainnet", "all-networks", "treasury"],
      answer: 0,
      explain: `The rule of thumb: a model holds only keys whose total loss you can shrug at. Testnet lumens are free from friendbot; a mainnet or treasury key inside an automated loop is an incident with a countdown on it.`,
    },
    {
      kind: "theory",
...
The model needs to check a balance, so it gets network access — narrowly, for that. A week later it needs to install a dependency, so the network stays open. Someone is debugging a mainnet issue and drops a real key into the environment "just for this run", and nobody removes it, because removing it is a task and nothing is currently broken.
...
The audit is cheap and nobody runs it: **list what the model holds today, and for each one, name the task that needed it.** Anything without a name in that column is a grant nobody remembers making.`,
    },
    {
      kind: "quiz",
      question: `You add open network access and write-anywhere to a model that already reads the repo, runs tests, writes in one directory and holds testnet keys. What did those two grants buy?`,
      options: [
...
Amateurs design what happens when the model is *right*. Engineers design what happens when it's **wrong** — because sometimes it will be.

- A failed check **blocks the merge**; it doesn't log a warning into the void.
- Retries have a **budget**, so a stuck model becomes a stopped model, not a bill.
...
For every step of the harness, ask one question: *"when this is wrong, what catches it?"* If the answer is "hopefully nothing goes wrong" — that's a wish, not a design.`,
    ...
    },
    {
      ...
      options: [
        ...
        "The prompt firmly instructs the model to be extremely careful and to double-check everything",
...
That's the discipline in one image: build the bench so that being wrong is *detectable* and being right is *provable* — for models and for humans.

**Next:** the words themselves — what the model actually sees on the bench.`,
    },
  ],
  testOut: [
    {
      ...
      options: [
        "A model should hold only keys whose total loss you can shrug at — friendbot lumens are free, a treasury key is an incident with a countdown",
    ...
    },
    {
      ...
      options: [
        ...
        "The prompt firmly instructs the model to be careful and double-check everything",
    ...
    {
      question: `You grant the model open network access and a directory it can write to anywhere. What did that actually buy?`,
      options: [
